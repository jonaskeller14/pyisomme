"""Fix two known metadata errors in the NHTSA test 09203 channel files
(D:\\20220211_pyisomme\\data\\nhtsa\\09203\\Channel\\09203.*).

Sibling of ``../14084/fix_channel_metadata.py`` — same two mistakes, different
occupants. 09203 is "NHTSA RESEARCH MDB 2010B into TOYOTA YARIS 2010", so the
instrumented occupants sit in **test object 2**, and the channel codes start
``21`` (object 2 / position 1) and ``24`` (object 2 / position 4) rather than
``11``/``13``.
...
1. Dummy field not populated. Byte range 11-12 of the 16-char ISO-MME channel
   ...
   instead of naming the dummy. For this test:
     - position 1 (code prefix "21") is a **THOR** -> "TH"
     - position 4 (code prefix "24") is a **Hybrid III** -> "H3"
   e.g. 21CHSTLEUP00DSX? -> 21CHSTLEUPTHDSX?

   Both are readable straight off the raw files: position 1 carries the four
   CRUX IR-TRACC chest deflections (``21CHSTLEUP/LELO/RIUP/RILO``, comments
   tagged ``[THORTEST]``), which only a THOR has, while position 4 carries the
   single ``24CHST000000DSXP`` of a Hybrid III.

2. Wrong unit on the chest deflection channels. The CHST/DS channels declare
   "Unit :m" but the recorded values are in micrometers -- e.g.
   ``21CHSTLEUP00DS0?`` peaks at 30270, and ``24CHST000000DSXP`` at -40598,
   which are 30.3 mm and -40.6 mm, the expected order of magnitude for a chest
   deflection. Declared as metres they would be off by a factor of a million.
   -> should be "Unit :μm".

   Only CHST/DS is touched. The abdomen and pelvis channels are in the same
   units but carry unusable ``21????????00…`` codes (NHTSA could not map them
   to ISO-MME), so nothing reads them and guessing at their location would be
...
DUMMY_BY_PREFIX = {
    "21": "TH",  # test object 2, position 1 (driver): THOR
    "24": "H3",  # test object 2, position 4: Hybrid III
}
