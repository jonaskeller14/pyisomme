"""Fix two known metadata errors in the NHTSA test 14084 channel files
(D:\\20220211_pyisomme\\data\\nhtsa\\14084\\Channel\\14084.*).

The raw NHTSA export has two mistakes in the "Channel code"/"Unit" header
lines of each channel file:

1. Dummy field not populated. Byte range 11-12 of the 16-char ISO-MME
   channel code (the "fine_location_3"/dummy slot) is left as the "00"
   placeholder instead of naming the dummy. For this test:
     - driver channels   (code prefix "11") use a Hybrid III 50th male -> "H3"
     - passenger channels (code prefix "13") use a Hybrid III 5th female -> "HF"
   e.g. 11HEADCG0000ACYP -> 11HEADCG00H3ACYP

2. Wrong unit on the chest deflection channels. The CHST/DS (chest
   displacement) channels declare "Unit :m" but the recorded values are
   actually in micrometers -> should be "Unit :μm".
...
DUMMY_BY_PREFIX = {
    "11": "H3",  # driver: Hybrid III 50th percentile male
    "13": "HF",  # passenger: Hybrid III 5th percentile female
}
