...
def build_parser() -> argparse.ArgumentParser:
    ...
    list_parser.add_argument("input_paths", nargs="+",
                             help="ISO-MME Path (.mme/folder/.zip/.tar/.tar.gz/...)")
    list_parser.add_argument("-c", "--codes", nargs="*", default=[],
                             help="Channel Code Patterns to filter ISO-MMEs")

    merge_parser = command_parsers.add_parser("merge", help="Merge ISO-MMEs")
    merge_parser.add_argument("output_path",
                              help="ISO-MME Path (.mme/folder/.zip/.tar/.tar.gz/...)")
    merge_parser.add_argument("input_paths", nargs="+",
                              help="ISO-MME Path (.mme/folder/.zip/.tar/.tar.gz/...)")
    merge_parser.add_argument("-c", "--codes", nargs="*", default=[],
                              help="Channel Code Patterns to filter ISO-MMEs")
    merge_parser.add_argument("--delete-duplicates", action="store_true",
                              help="Delete duplicate channels (same channel code)")
    merge_parser.add_argument("--delete-filter-duplicates", action="store_true",
    ...
    merge_parser.add_argument("--auto-offset-y", action="store_true",
                              help="Apply y-offset with value from t=0")
    merge_parser.add_argument("--append", action="store_true",
                              help="Append channels to ISO-MME if output_path exists")
    merge_parser.add_argument("--resample", nargs=3, type=float,
                              metavar=("START", "STEP", "STOP"),
                              help="Resampling by linear interpolation (no extrapolation)")
    merge_parser.add_argument("--crop", nargs=2, type=float, metavar=("START", "STOP"),
    ...
    merge_parser.add_argument("--delete-info", action="store_true",
    ...
    set_parser.add_argument("input_paths", nargs="+",
                            help="Writable ISO-MME path(s) (.mme/folder/.zip/.tar/.tar.gz)")
    set_parser.add_argument("field", choices=SET_FIELDS,
                            help="Unit or channel-code field to set")
    set_parser.add_argument("value",
                            help="New metadata value; unit relabels data without converting values")
    set_parser.add_argument("-c", "--codes", nargs="+", required=True,
    ...
    convert_parser.add_argument("input_paths", nargs="+",
                                help="Writable ISO-MME path(s) (.mme/folder/.zip/.tar/.tar.gz)")
    convert_parser.add_argument("field", choices=("unit",),
                                help="Metadata field whose values and data should be converted")
    convert_parser.add_argument("value", help="Target unit compatible with every selected channel")
    convert_parser.add_argument("-c", "--codes", nargs="+", required=True,
                                metavar="CODE_PATTERN",
                                help="Quoted channel-code pattern(s); use -c '*' to select every channel")

    report_parser = command_parsers.add_parser("report", help="Create a Report")
    report_parser.add_argument("report_name", choices=[report.__name__ for report in REPORTS],
                               help="Report name")
    report_parser.add_argument("report_path", help="Report Path (.pptx)")
    report_parser.add_argument("input_paths", nargs="+",
                               help="ISO-MME Path (.mme/folder/.zip/.tar/.tar.gz/...)")
    report_parser.add_argument("--template", help="Path to Template (.pptx)")
    report_parser.add_argument("--crop", nargs=2, type=float, metavar=("START", "STOP"),
                               help="Crop ISO-MME channels to x-min to x-max e.g. (--crop 0.0 0.15)")

    plot_parser = command_parsers.add_parser("plot", help="Plot Channels")
    plot_parser.add_argument("input_paths", nargs="+",
                             help="ISO-MME Path (.mme/folder/.zip/.tar/.tar.gz/...)")
    plot_parser.add_argument("-c", "--codes", nargs="*", default=[],
                             help="Channel Code Patterns to select Channel to plot")
    plot_parser.add_argument("--calculate", action="store_true", help="Plot calculated Channel.")
    plot_parser.add_argument("-n", "--n-channels", type=int, dest="n",
                             help="Limit Number of channels to plot")
    plot_parser.add_argument("-x", "--xlim", nargs=2, type=float,
                             metavar=("START", "STOP"), help="X-Axis Range [in ms]")
    plot_parser.add_argument("-y", "--ylim", nargs=2, type=float,
                             metavar=("START", "STOP"), help="Y-Axis Range")
    plot_parser.add_argument("--legend-off", dest="legend", action="store_false",
                             help="Hide legend")
    return parser
...
def _writable_input(path: Path) -> bool:
    ...
    return path.suffix == "" or path.suffix.lower() in {".mme", ".zip", ".tar"} or (
        len(suffixes) >= 2 and suffixes[-2:] == [".tar", ".gz"]
    )


def _load_selected(input_paths: list[str], code_patterns: list[str]
    ...
    for path in paths:
        ...
        channels = [
            channel for channel in isomme.channels
...
def _print_changes(path: Path, field: str, channels: list[Channel],
...
def _set_channel_metadata(input_paths: list[str], field: str, value: str,
    ...
    if field == "unit":
        ...
        for _, _, channels in selected:
            changes_by_path.append([
                ...
                for channel in channels if channel.unit != new_unit
            ])
        for _, _, channels in selected:
        ...
        changes_by_path = [
            [(str(channel.code), str(channel.code), str(new_code))
             for channel, new_code in replacements if channel.code != new_code]
        ...
        for (path, isomme, _), replacements in zip(selected, replacements_by_path):
            ...
            new_duplicates = sorted(
                str(code) for code, count in new_code_counts.items()
                if count > 1 and count > old_code_counts[code]
            )
            if new_duplicates:
                logger.warning("Code rewrite in '%s' created duplicate channel code(s): %s",
                               path, ", ".join(new_duplicates))
...
def _convert_channel_units(input_paths: list[str], value: str,
                           code_patterns: list[str]) -> None:
    selected = _load_selected(input_paths, code_patterns)
    new_unit = Unit(value)
...
def main(argv: list[str] | None = None) -> None:
    ...
    if options.command == "list":
        for isomme in [Isomme().read(input_path) for input_path in options.input_paths]:
            ...
            channels = isomme.channels if len(options.codes) == 0 else isomme.get_channels(
                *options.codes, filter=False, calculate=False, integrate=False, differentiate=False
            )
            for channel in channels:
                print(channel.code)
        ...
        for channel in merged_isomme.channels:
            channel.set_code(test_object=options.test_object, position=options.position,
                             ...
                             direction=options.direction, filter_class=options.filter_class)
        ...
        try:
            _set_channel_metadata(options.input_paths, options.field, options.value, options.codes)
        ...
        if options.calculate:
            ...
            isomme_list = [Isomme().read(input_path, "??TIRS??????????", *options.codes)
