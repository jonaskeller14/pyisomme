...
def calculate_neck_MOCx(
    channel_Mx: Channel, channel_Fy: Channel, d: float | None = None
    ...
    if d is None:
        dummys = list(
            {channel_Mx.code.fine_location_3, channel_Fy.code.fine_location_3}
        )
        if len(dummys) != 1:
    ...
    channel_calc.data = pd.DataFrame(
        data=[channel.get_data()[np.argmax(np.abs(channel.get_data()))]],
...
@debug_logging(logger)
def calculate_neck_MOCy(
    channel_My: Channel, channel_Fx: Channel, d: float | None = None
) -> tuple[Channel, Channel]:
    """
    References:
    - references/Euro-NCAP/tb-021-data-acquisition-and-injury-calculation-v402.pdf
    ...
    if d is None:
        dummys = list(
            {channel_My.code.fine_location_3, channel_Fx.code.fine_location_3}
        )
        if len(dummys) != 1:
            raise ValueError(f"Multiple dummy types found: {dummys}")
        dummy = dummys[0]
        ...
        d = {"WS": 0.0195, "H3": 0.01778, "HF": 0.01778}[dummy]  # [m]
