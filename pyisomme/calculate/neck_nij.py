...
def calculate_neck_nij(
    ...
    c_nij = Channel(
        code=c_fz.code.set(
            main_location="NIJC",
            fine_location_1="OP" if oop else "IP",
            fine_location_2="00",
            physical_dimension="00",
            direction="Y",
        ),
        data=pd.DataFrame(
            np.nansum([data_ncf, data_nce, data_ntf, data_nte], axis=0), index=t
        ),
        unit="1",
        info={
    ...
    c_ncf = Channel(
        code=c_nij.code.set(fine_location_2="CF"),
        data=data_ncf,
        unit=c_nij.unit,
        info=c_nij.info,
    )
    c_nce = Channel(
        code=c_nij.code.set(fine_location_2="CE"),
        data=data_nce,
        unit=c_nij.unit,
        info=c_nij.info,
    )
    c_ntf = Channel(
        code=c_nij.code.set(fine_location_2="TF"),
        data=data_ntf,
        unit=c_nij.unit,
        info=c_nij.info,
    )
    c_nte = Channel(
        code=c_nij.code.set(fine_location_2="TE"),
        data=data_nte,
        unit=c_nij.unit,
        info=c_nij.info,
    )
    ...
    c_nij_x = Channel(
        code=c_nij.code.set(filter_class="X"),
        data=pd.DataFrame(
            [np.max(c_nij.get_data())],
            index=[c_nij.data.index[int(np.argmax(c_nij.get_data()))]],
        ),
        unit=c_nij.unit,
        info=c_nij.info.update(
            {
                ".Time": c_nij.data.index[int(np.argmax(c_nij.get_data()))],
    ...
    c_ncf_x = Channel(
        code=c_ncf.code.set(filter_class="X"),
        data=pd.DataFrame(
            [np.max(c_ncf.get_data())],
            index=[c_ncf.data.index[int(np.argmax(c_ncf.get_data()))]],
        ),
        unit=c_nij.unit,
        info=c_nij_x.info.update(
            {"Time": c_ncf.data.index[int(np.argmax(c_ncf.get_data()))]}
        ),
    )
    c_nce_x = Channel(
        code=c_nce.code.set(filter_class="X"),
        data=pd.DataFrame(
            [np.max(c_nce.get_data())],
            index=[c_nce.data.index[int(np.argmax(c_nce.get_data()))]],
        ),
        unit=c_nce.unit,
        info=c_nij_x.info.update(
            {"Time": c_nce.data.index[int(np.argmax(c_nce.get_data()))]}
        ),
    )
    c_ntf_x = Channel(
        code=c_ntf.code.set(filter_class="X"),
        data=pd.DataFrame(
            [np.max(c_ntf.get_data())],
            index=[c_ntf.data.index[int(np.argmax(c_ntf.get_data()))]],
        ),
        unit=c_ntf.unit,
        info=c_nij_x.info.update(
            {"Time": c_ntf.data.index[int(np.argmax(c_ntf.get_data()))]}
        ),
    )
    c_nte_x = Channel(
        code=c_nte.code.set(filter_class="X"),
        data=pd.DataFrame(
            [np.max(c_nte.get_data())],
            index=[c_nte.data.index[int(np.argmax(c_nte.get_data()))]],
        ),
        unit=c_nte.unit,
        info=c_nij_x.info.update(
            {"Time": c_nte.data.index[int(np.argmax(c_nte.get_data()))]}
        ),
    )
