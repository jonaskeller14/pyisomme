...
class Correlation_ISO18571:
    def __init__(
        self, reference_channel: Channel, comparison_channel: Channel, *args, **kwargs
    ):
        ...
        if not np.all(np.abs(np.diff(time) - 0.0001) < 10e-6):
            logger.warning(
                "Time resolution of 10 kHz required. Channel Data will be adjusted by interpolation."
            )
            t_start = max(
                comparison_channel.data.index[0], reference_channel.data.index[0]
            )
            t_end = min(
                comparison_channel.data.index[-1], reference_channel.data.index[-1]
            )
            time = np.arange(t_start, t_end, 0.0001)

        reference_curve = np.vstack((time, reference_channel.get_data(t=time))).T
        comparison_curve = np.vstack(
            (time, comparison_channel.get_data(t=time, unit=reference_channel.unit))
        ).T

        self.iso18571 = ISO18571(
            *args,
