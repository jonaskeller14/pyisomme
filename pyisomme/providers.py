...
class AggregatePairProvider(ChannelProvider):
    ...
    def build(self, isomme: Isomme, code: Code) -> Channel | None:
        channels = _all_not_none(
            [
                isomme.get_channel(code.set(**{self.vary: member}))
        ...
        data = [first.get_data(t=t)] + [
            channel.get_data(t=t, unit=first.unit) for channel in channels[1:]
        ]
        if self.agg == "min":
        ...
        return Channel(
            code=first.code.set(**{self.vary: "00"}),
...
def _build_resultant(isomme: Isomme, code: Code) -> Channel | None:
    channel_xyz = _all_not_none(
        [isomme.get_channel(code.set(direction=direction)) for direction in "XYZ"]
    )
    if channel_xyz is not None:
        return calculate_resultant(*channel_xyz)
    channel_123 = _all_not_none(
        [isomme.get_channel(code.set(direction=direction)) for direction in "123"]
    )
...
def _build_bric(isomme: Isomme, code: Code) -> Channel | None:
    channel_head_av_xyz = _all_not_none(
        [
            isomme.get_channel(
                code.set(
                    main_location="HEAD",
                    physical_dimension="AV",
                    direction=direction,
                    filter_class="D",
                )
            )
...
def _build_hic(isomme: Isomme, code: Code) -> Channel | None:
    head_channel = isomme.get_channel(
        code.set(
            main_location="HEAD",
            fine_location_1="??",
            fine_location_2="00",
...
def _build_xms(isomme: Isomme, code: Code) -> Channel | None:
    ...
    if channel is not None:
        ...
        return calculate_xms(
            channel, min_delta_t=int(code.fine_location_2[0]), method=method
        )
    return None
    ...
    if code.filter_class == "X":
        ...
        channel_xyz = _all_not_none(
            [
                isomme.get_channel(
                    code.set(
                        fine_location_1="00", fine_location_2="00", direction=direction
                    ),
                    code.set(
                        fine_location_1="CG", fine_location_2="00", direction=direction
                    ),
                )
                for direction in "XYZ"
            ]
...
def _build_neck_total_moment(isomme: Isomme, code: Code) -> Channel | None:
    if code.fine_location_1 == "UP":
        if code.direction == "X":
            if code.filter_class == "X":
                channel_mx = isomme.get_channel(
                    code.set(
                        main_location="NECK",
                        physical_dimension="MO",
                        direction="X",
                        filter_class="B",
                    )
                )
                channel_fy = isomme.get_channel(
                    code.set(
                        main_location="NECK",
                        physical_dimension="FO",
                        direction="Y",
                        filter_class="B",
                    )
                )
                ...
                channel_mx = isomme.get_channel(
                    code.set(
                        main_location="NECK", physical_dimension="MO", direction="X"
                    )
                )
                channel_fy = isomme.get_channel(
                    code.set(
                        main_location="NECK", physical_dimension="FO", direction="Y"
                    )
                )
                if channel_mx is not None and channel_fy is not None:
            ...
            if code.filter_class == "X":
                ...
                channel_my = isomme.get_channel(
                    code.set(
                        main_location="NECK", physical_dimension="MO", direction="Y"
                    )
                )
                channel_fx = isomme.get_channel(
                    code.set(
                        main_location="NECK", physical_dimension="FO", direction="X"
                    )
                )
                if channel_my is not None and channel_fx is not None:
    ...
    if code.filter_class == "X":
        ...
        if c_fz is not None and c_mocy is not None:
            if code.fine_location_2 == "00":
                return calculate_neck_nij(
                    c_fz, c_mocy, oop=code.fine_location_1 == "OP"
                )[5]
...
def _build_vc(isomme: Isomme, code: Code) -> Channel | None:
    ...
    if code.main_location == "VCCR":
        if code.filter_class == "X":
            channel = isomme.get_channel(
                code.set(
                    main_location="CHST", physical_dimension="DS", filter_class="C"
                )
            )
            if channel is not None:
            ...
            channel = isomme.get_channel(
                code.set(
                    main_location="TRRI", physical_dimension="DS", filter_class="C"
                )
            )
            if channel is not None:
                return calculate_vc(channel)[1]
            channel = isomme.get_channel(
                code.set(
                    main_location="RIBS", physical_dimension="DS", filter_class="C"
                )
            )
            if channel is not None:
                return calculate_vc(channel)[1]
            ...
            channel = isomme.get_channel(
                code.set(main_location="CHST", physical_dimension="DS")
            )
            if channel is not None:
                return calculate_vc(channel)[0]
            channel = isomme.get_channel(
                code.set(main_location="TRRI", physical_dimension="DS")
            )
            if channel is not None:
                return calculate_vc(channel)[0]
            channel = isomme.get_channel(
                code.set(main_location="RIBS", physical_dimension="DS")
            )
            if channel is not None:
                return calculate_vc(channel)[0]
        ...
        if code.fine_location_2 == "00":
            if code.filter_class == "X":
                ...
                if (
                    channel_up is not None
                    and channel_mi is not None
                    and channel_lo is not None
                ):
                    t = time_intersect(channel_up, channel_mi, channel_lo)
                    values = np.array(
                        [
                            channel_up.get_data(t=t),
                    ...
                    return Channel(
                        code=channel_up.code.set(fine_location_2="00"),
                        data=pd.DataFrame(new_values, index=t),
                        unit=channel_up.unit,
                        info=channel_up.info,
                    )

                channel_01 = isomme.get_channel(
                    code.set(fine_location_2="01", filter_class="C")
                )
                channel_02 = isomme.get_channel(
                    code.set(fine_location_2="02", filter_class="C")
                )
                channel_03 = isomme.get_channel(
                    code.set(fine_location_2="03", filter_class="C")
                )
                if (
                    channel_01 is not None
                    and channel_02 is not None
                    and channel_03 is not None
                ):
                    value_01 = channel_01.get_data()[
                        np.argmax(np.abs(channel_01.get_data()))
                    ]
                    value_02 = channel_02.get_data(unit=channel_01.unit)[
                        np.argmax(np.abs(channel_02.get_data(unit=channel_01.unit)))
                    ]
                    value_03 = channel_03.get_data(unit=channel_01.unit)[
                        np.argmax(np.abs(channel_03.get_data(unit=channel_01.unit)))
                    ]
                    value = np.array([value_01, value_02, value_03])[
                        np.argmax(np.abs([value_01, value_02, value_03]))
                    ]
                    return Channel(
                        code=channel_01.code.set(
                            fine_location_2="00", filter_class="X"
                        ),
                        data=pd.DataFrame([value]),
                        unit=channel_01.unit,
                    )

                channel_LO = isomme.get_channel(
                    code.set(fine_location_2="LO", filter_class="C")
                )
                channel_UP = isomme.get_channel(
                    code.set(fine_location_2="UP", filter_class="C")
                )
                if channel_LO is not None and channel_UP is not None:
                    value_left = channel_LO.get_data()[
                        np.argmax(np.abs(channel_LO.get_data()))
                    ]
                    value_right = channel_UP.get_data(unit=channel_LO.unit)[
                        np.argmax(np.abs(channel_UP.get_data(unit=channel_LO.unit)))
                    ]
                    value = np.array([value_left, value_right])[
                        np.argmax(np.abs([value_left, value_right]))
                    ]
                    return Channel(
                        code=channel_LO.code.set(
                            fine_location_2="00", filter_class="X"
                        ),
                        data=pd.DataFrame([value]),
                ...
                if (
                    ...
                    new_values = values[idx_max_abs, np.arange(values.shape[1])]
                    return Channel(
                        code=channel_01.code.set(fine_location_2="00"),
                        data=pd.DataFrame(new_values, index=t),
                        unit=channel_01.unit,
                    )
                ...
                if channel_LO is not None and channel_UP is not None:
                    ...
                    values = np.array(
                        [
                            channel_LO.get_data(t=t),
                            channel_UP.get_data(t=t, unit=channel_LO.unit),
                        ]
                    )
                    idx_max_abs = np.argmax(np.abs(values), axis=0)
                    new_values = values[idx_max_abs, np.arange(values.shape[1])]
                    return Channel(
                        code=channel_LO.code.set(fine_location_2="00"),
                        data=pd.DataFrame(new_values, index=t),
                        unit=channel_LO.unit,
                    )
    ...
    if code.main_location == "VCAR":
        if code.filter_class == "X":
            channel = isomme.get_channel(
                code.set(
                    main_location="ABDO", physical_dimension="DS", filter_class="C"
                )
            )
            if channel is not None:
                return calculate_vc(channel)[1]
            channel = isomme.get_channel(
                code.set(
                    main_location="ABRI", physical_dimension="DS", filter_class="C"
                )
            )
            if channel is not None:
                return calculate_vc(channel)[1]
            ...
            channel = isomme.get_channel(
                code.set(main_location="ABDO", physical_dimension="DS")
            )
            if channel is not None:
                return calculate_vc(channel)[0]
            channel = isomme.get_channel(
                code.set(main_location="ABRI", physical_dimension="DS")
            )
            if channel is not None:
                return calculate_vc(channel)[0]
        ...
        if code.fine_location_2 == "00":
            if code.filter_class == "X":
                ...
                if channel_01 is not None and channel_02 is not None:
                    value_left = channel_01.get_data()[
                        np.argmax(np.abs(channel_01.get_data()))
                    ]
                    value_right = channel_02.get_data(unit=channel_01.unit)[
                        np.argmax(np.abs(channel_02.get_data(unit=channel_01.unit)))
                    ]
                    value = np.array([value_left, value_right])[
                        np.argmax(np.abs([value_left, value_right]))
    ...
    if code.fine_location_1 == "00":
        if code.filter_class == "X":
            channel_left = isomme.get_channel(
                code.set(fine_location_1="LE", filter_class="C")
            )
            channel_right = isomme.get_channel(
                code.set(fine_location_1="RI", filter_class="C")
            )
            if channel_left is not None and channel_right is not None:
                value_left = channel_left.get_data()[
                    np.argmax(np.abs(channel_left.get_data()))
                ]
                value_right = channel_right.get_data(unit=channel_left.unit)[
                    np.argmax(np.abs(channel_right.get_data(unit=channel_left.unit)))
                ]
                value = np.array([value_left, value_right])[
                    np.argmax(np.abs([value_left, value_right]))
                ]
                return Channel(
                    code=channel_left.code.set(fine_location_1="00", filter_class="X"),
                    data=pd.DataFrame([value]),
            ...
            if channel_left is not None and channel_right is not None:
                ...
                new_values = values[idx_max_abs, np.arange(values.shape[1])]
                return Channel(
                    code=channel_left.code.set(fine_location_1="00"),
                    data=pd.DataFrame(new_values, index=t),
                    unit=channel_left.unit,
                )
    return None
...
def _build_kthc_min(isomme: Isomme, code: Code) -> Channel | None:
    ...
    if channel_left is not None and channel_right is not None:
        idx_min = np.argmin(
            [channel_left.get_data()[0], channel_right.get_data()[0]], axis=0
        )
        channel_min = [channel_left, channel_right][idx_min]
        return Channel(
            code=channel_min.code.set(fine_location_1="00"),
...
def _build_kthc_femur_impulse(isomme: Isomme, code: Code) -> Channel | None:
    channel_foz = isomme.get_channel(
        code.set(main_location="FEMR", physical_dimension="FO", filter_class="B")
    )
...
def _build_tibia_index(isomme: Isomme, code: Code) -> Channel | None:
    ...
    channel_MOX = isomme.get_channel(
        code.set(
            main_location="TIBI",
            fine_location_1=fine_location_1,
            fine_location_2=fine_location_2,
            physical_dimension="MO",
            direction="X",
        )
    )
    channel_MOY = isomme.get_channel(
        code.set(
            main_location="TIBI",
            fine_location_1=fine_location_1,
            fine_location_2=fine_location_2,
...
def _build_tibia_index_using_total_moment(isomme: Isomme, code: Code) -> Channel | None:
    ...
    if fine_location_2 == "UP":
        channel_MOY_total = calculate_adjusted_upper_tibia_moment_My(
            channel_MOY=channel_MOY, channel_FOZ=channel_FOZ
        )
    elif fine_location_2 == "LO":
        channel_MOY_total = calculate_adjusted_lower_tibia_moment_My(
            channel_MOY=channel_MOY, channel_FOZ=channel_FOZ
        )
    else:
        return None
    ...
    return calculate_tibia_index_using_total_moment(
        channel_MOX, channel_MOY_total, channel_FOZ
    )


def _build_chest_pc_score(isomme: Isomme, code: Code) -> Channel | None:
    channel_le_up_ds = isomme.get_channel(
        code.set(fine_location_1="LE", fine_location_2="UP")
    )
    channel_ri_up_ds = isomme.get_channel(
        code.set(fine_location_1="RI", fine_location_2="UP")
    )
    channel_le_lo_ds = isomme.get_channel(
        code.set(fine_location_1="LE", fine_location_2="LO")
    )
    channel_ri_lo_ds = isomme.get_channel(
        code.set(fine_location_1="RI", fine_location_2="LO")
    )
    if (
        channel_le_up_ds is not None
        and channel_ri_up_ds is not None
        and channel_le_lo_ds is not None
        and channel_ri_lo_ds is not None
    ):
        return calculate_chest_pc_score(
            channel_le_up_ds=channel_le_up_ds,
...
def _build_chst_irtracc_min(isomme: Isomme, code: Code) -> Channel | None:
    channels = _all_not_none(
        [
            isomme.get_channel(
                code.set(
                    fine_location_1=fine_location_1, fine_location_2=fine_location_2
                )
            )
            for fine_location_1, fine_location_2 in (
                ("LE", "UP"),
                ("RI", "UP"),
                ("LE", "LO"),
                ("RI", "LO"),
    ...
    if channels is not None and all(
        channel.code.fine_location_3 in ("TH", "T3", "00", "??") for channel in channels
    ):
        time = time_intersect(*channels)
        values = np.min(
            [channel.get_data(t=time, unit=channels[0].unit) for channel in channels],
            axis=0,
        )
        return Channel(
            code=channels[0].code.set(fine_location_1="00", fine_location_2="00"),
...
def _build_abdo_irtracc_min(isomme: Isomme, code: Code) -> Channel | None:
    channels = _all_not_none(
        [
            ...
            for fine_location_1, fine_location_2 in (("LE", "00"), ("RI", "00"))
        ]
    )
    if channels is not None and all(
        channel.code.fine_location_3 in ("TH", "T3", "00", "??") for channel in channels
...
def _build_thor_irtracc(isomme: Isomme, code: Code) -> Channel | None:
    if code.physical_dimension == "DC":
        delta = (
            15.65
            if code.fine_location_2 == "UP"
            else -15.65
            if code.fine_location_2 == "LO"
            else 0
        )  # [mm]
        if code.direction == "X":
            channel_dc0 = isomme.get_channel(code.set(direction="0"))
            channel_any = isomme.get_channel(
                code.set(physical_dimension="AN", direction="Y")
            )
            channel_anz = isomme.get_channel(
                code.set(physical_dimension="AN", direction="Z")
            )
            if (
                channel_dc0 is not None
                and channel_any is not None
                and channel_anz is not None
            ):
                time = time_intersect(channel_dc0, channel_any, channel_anz)
                channel_any = channel_any.adjust_to_range(
                    target_range=(-45, 45), unit="deg"
                )
                channel_anz = channel_anz.adjust_to_range(
                    target_range=(-45, 45), unit="deg"
                )
                values = delta * np.sin(
                    channel_any.get_data(t=time, unit="rad")
                ) + channel_dc0.get_data(t=time, unit="mm") * np.cos(
                    channel_any.get_data(t=time, unit="rad")
                ) * np.cos(channel_anz.get_data(t=time, unit="rad"))
                return Channel(
                    code=channel_dc0.code.set(direction="X"),
                    data=pd.DataFrame(values, index=time),
                    unit="mm",
                )
        if code.direction == "Y":
            ...
            if channel_dc0 is not None and channel_anz is not None:
                ...
                values = channel_dc0.get_data(t=time, unit="mm") * np.sin(
                    channel_anz.get_data(t=time, unit="rad")
                )
                return Channel(
                    code=channel_dc0.code.set(direction="Y"),
                    data=pd.DataFrame(values, index=time),
                    unit="mm",
                )
        if code.direction == "Z":
            ...
            if (
                ...
                values = delta * np.cos(
                    (channel_any).get_data(t=time, unit="rad")
                ) - channel_dc0.get_data(t=time, unit="mm") * np.sin(
                    channel_any.get_data(t=time, unit="rad")
                ) * np.cos(channel_anz.get_data(t=time, unit="rad"))
                return Channel(
                    code=channel_dc0.code.set(direction="Z"),
                    data=pd.DataFrame(values, index=time),
                    unit="mm",
                )
    if code.physical_dimension == "DS":
        if code.direction == "0":
            ...
            if channel_dc0 is not None:
                return (channel_dc0 - channel_dc0.get_data(t=0)).set_code(
                    physical_dimension="DS"
                )
        if code.direction == "X":
            channel_dcx = isomme.get_channel(
                code.set(physical_dimension="DC", direction="X")
            )
            if channel_dcx is not None:
                return (channel_dcx - channel_dcx.get_data(t=0)).set_code(
                    physical_dimension="DS"
                )
        if code.direction == "Y":
            channel_dcy = isomme.get_channel(
                code.set(physical_dimension="DC", direction="Y")
            )
            if channel_dcy is not None:
                return (channel_dcy - channel_dcy.get_data(t=0)).set_code(
                    physical_dimension="DS"
                )
        if code.direction == "Z":
            channel_dcz = isomme.get_channel(
                code.set(physical_dimension="DC", direction="Z")
            )
            if channel_dcz is not None:
                return (channel_dcz - channel_dcz.get_data(t=0)).set_code(
                    physical_dimension="DS"
                )
    return None
...
def _build_worldsid_trri_min(isomme: Isomme, code: Code) -> Channel | None:
    ...
    if channel_01 is not None and channel_02 is not None and channel_03 is not None:
        ...
        values = np.min(
            [
                channel.get_data(t=time, unit=channel_01.unit)
                for channel in (channel_01, channel_02, channel_03)
            ],
            axis=0,
        )
        return Channel(
...
def _build_worldsid_abri_min(isomme: Isomme, code: Code) -> Channel | None:
    ...
    if channel_01 is not None and channel_02 is not None:
        ...
        values = np.min(
            [
                channel.get_data(t=time, unit=channel_01.unit)
                for channel in (channel_01, channel_02)
            ],
            axis=0,
        )
        return Channel(
...
def _build_worldsid_rib_irtracc(isomme: Isomme, code: Code) -> Channel | None:
    if code.physical_dimension == "DC" and code.direction == "Y":
        channel_dc0 = isomme.get_channel(
            code.set(physical_dimension="DC", direction="0")
        )
        channel_anz = isomme.get_channel(
            code.set(physical_dimension="AN", direction="Z")
        )
        if channel_dc0 is not None and channel_anz is not None:
            ...
            values = channel_dc0.get_data(t=t) * np.cos(
                channel_anz.get_data(t=t, unit="rad")
            )
            return Channel(
    ...
    if code.physical_dimension == "DS":
        ...
        if channel_dc is not None:
            return (channel_dc - channel_dc.get_data(t=0)).set_code(
                physical_dimension="DS"
            )
    return None
...
PROVIDERS: list[ChannelProvider] = [
    # Resultant
    _FnProvider(
        lambda c: c.direction == "R" and c.filter_class != "X", _build_resultant
    ),
    # BrIC
    _FnProvider(
        lambda c: c.main_location == "BRIC" and c.filter_class == "X", _build_bric
    ),
    # HIC
    _FnProvider(
        lambda c: c.main_location == "HICR" and c.filter_class == "X", _build_hic
    ),
    # xms
    _FnProvider(
        lambda c: (
            fnmatch.fnmatch(c.fine_location_2, "[0-9][CS]") and c.filter_class == "X"
        ),
        _build_xms,
    ),
    # Damage
    _FnProvider(
        lambda c: (
            c.fine_location_1 == "DA"
            and c.fine_location_2 == "MA"
            and c.physical_dimension == "AA"
        ),
        _build_damage,
    ),
    # Neck Total Moment
    _FnProvider(lambda c: c.main_location == "TMON", _build_neck_total_moment),
    # Neck NIJ
    _FnProvider(lambda c: c.main_location == "NIJC", _build_nij),
    # Shoulder Lateral Force (Y) (min/max of left and right)
    AggregatePairProvider(
        lambda c: (
            c.main_location == "SHLD"
            and c.fine_location_1 == "00"
            and c.physical_dimension == "FO"
            and c.direction == "Y"
        ),
        vary="fine_location_1",
        members=("LE", "RI"),
        agg="max_abs",
        keep_info=True,
    ),
    # Viscous Criterion (Chest and Abdomen)
    _FnProvider(lambda c: c.main_location in ("VCCR", "VCAR"), _build_vc),
    # ES-2 / ES-2re Abdomen Force (Min. of front/middle/rear)
    AggregatePairProvider(
        lambda c: (
            c.main_location == "ABDO"
            and c.fine_location_2 == "00"
            and c.physical_dimension == "FO"
            and c.direction == "Y"
        ),
        vary="fine_location_2",
        members=("RE", "MI", "FR"),
        agg="min",
        keep_info=True,
    ),
    ...
    _FnProvider(
        lambda c: (
            c.main_location == "KTHC"
            and c.physical_dimension == "IM"
            and c.fine_location_1 == "00"
            and c.filter_class == "X"
        ),
        _build_kthc_min,
    ),
    # KTH femur impulse
    ...
    AggregatePairProvider(
        lambda c: (
            c.main_location == "KNSL"
            and c.fine_location_1 == "00"
            and c.fine_location_2 == "00"
            and c.physical_dimension in ("FO", "DS")
            and c.direction == "X"
        ),
        vary="fine_location_1",
        members=("LE", "RI"),
    ...
    _FnProvider(
        lambda c: (
            c.main_location == "TIIN"
            and c.fine_location_2 == "00"
            and c.physical_dimension == "00"
            and c.direction == "0"
        ),
        _build_tibia_index,
    ),
    ...
    _FnProvider(
        lambda c: (
            c.main_location == "CHST"
            and c.fine_location_1 == "00"
            and c.fine_location_2 == "PC"
            and c.physical_dimension == "DS"
            and c.filter_class != "X"
        ),
    ...
    _FnProvider(
        lambda c: (
            (
                (
                    c.main_location == "CHST"
                    and c.fine_location_1 in ("LE", "RI")
                    and c.fine_location_2 in ("UP", "LO")
                )
                or (
                    c.main_location == "ABDO"
                    and c.fine_location_1 in ("LE", "RI")
            ...
            and c.fine_location_3 in ("TH", "T3")
        ),
        _build_thor_irtracc,
    ),
    ...
    _FnProvider(
        lambda c: (
            c.main_location == "TRRI"
            and c.fine_location_2 == "00"
            and c.fine_location_3 in ("WS", "??")
            and c.physical_dimension == "DS"
        ),
    ...
    _FnProvider(
        lambda c: (
            c.main_location in ("TRRI", "ABRI") and c.fine_location_3 in ("WS", "??")
        ),
    ...
    AggregatePairProvider(
        lambda c: (
            c.main_location == "FOOT"
            and c.fine_location_1 == "00"
            and c.physical_dimension == "AC"
            and c.direction == "R"
        ),
        vary="fine_location_1",
        members=("LE", "RI"),
    ...
    _FnProvider(
        lambda c: (
            c.fine_location_1 == "0O"
            and c.fine_location_2 == "LC"
            and c.physical_dimension == "VE"
        ),
        _build_olc,
    ),
]
