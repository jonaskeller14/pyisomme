...
class TestLimits(unittest.TestCase):
    ...
    def test_get_limit_idx(self):
        c1 = pyisomme.Channel(code="?" * 16, unit="1", data=pd.DataFrame([2.9, 0, 0, 1.9, 1.9, -1.5]))
        c2 = pyisomme.Channel(code="?" * 16, unit="1", data=pd.DataFrame([4, 0, 0, 3, 0, 0]))
        c3 = pyisomme.Channel(code="?" * 16, unit="1", data=pd.DataFrame([7, 7, 6, 5, 5, 5]))
