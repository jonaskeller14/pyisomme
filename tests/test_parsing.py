...
from pyisomme.parsing import parse_mme, parse_xxx, resolve_time_axis, get_normalization_notes
...
class Fixture:
    ...
    class EncodingContainer(unittest.TestCase):
        ...
        def check_structure(self, isomme):
            ...
            self.assertEqual([channel.code for channel in isomme.channels],
        ...
        def check_channels(self, isomme):
            ...
            self.assertTrue(np.isnan(values[DROPPED_SAMPLE_IDX]), "NOVALUE must become NaN")
            self.assertEqual(values[0], 0.0)
            # A colon inside a channel-header value, same trap as in the .mme.
            self.assertEqual([v for k, v in chest.info if k == "Comments"],
        ...
        def check_text(self, isomme):
            ...
            self.assertEqual([v for k, v in isomme.test_info if k == "Comments"],
                             self.comments)
            _, head, chest = isomme.channels
            self.assertEqual(head.get_info("Name of the channel"), self.head_channel_name)
            self.assertEqual(chest.get_info("Name of the channel"), self.chest_channel_name)
...
class TestUtf8(Fixture.EncodingContainer):
    ...
    def test_bom_does_not_leak_into_the_first_keyword(self):
        ...
        self.assertTrue(raw.startswith(b"\xef\xbb\xbf"), "fixture must carry a UTF-8 BOM")
